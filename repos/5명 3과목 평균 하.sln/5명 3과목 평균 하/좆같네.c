...
*/
int ave_personal(int arr1[][3], int arr2[]);
int ave_compare(int arr[], int* general);
int main() {
	...
	int ave_general;
	ave_compare(ave_score, &ave_general); //평균을 비교해서 높은 사람부터 출력
	printf("--------%d \n", ave_general);
	for (i = 0; i < 5; i++) {
		printf("%d번째로 높은 성적 평균: %d \n", i + 1, ave_score[i]);
		if (arr[i] >= ave_general) { //평균에 대해 합불 출력 
			printf("----합격\n");
		}
		if (arr[i] < ave_general) {
			printf("----불합격 \n");
		}
	}
